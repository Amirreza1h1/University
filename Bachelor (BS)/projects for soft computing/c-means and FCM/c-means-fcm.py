...
np.random.seed(42)
data_1 = np.random.randn(150, 2) * 1.5 + [2, 2]   # Cluster 1
data_2 = np.random.randn(150, 2) * 1.5 + [7, 7]   # Cluster 2
data_3 = np.random.randn(150, 2) * 1.5 + [12, 2]  # Cluster 3
...
def find_optimal_clusters_fcm(data, max_clusters):
    fpcs = []
    for n in range(2, max_clusters + 1):
        cntr, u, u0, d, jm, p, fpc = fuzz.cluster.cmeans(data.T, c=n, m=2, error=0.005, maxiter=1000)
        fpcs.append((n, fpc))
    return fpcs
...
cntr, u, u0, d, jm, p, fpc = fuzz.cluster.cmeans(data.T, c=best_fcm_clusters, m=2, error=0.005, maxiter=1000)

fcm_labels = np.argmax(u, axis=0)

# Step 5: Visualize FCM membership probabilities
def plot_fcm_membership(ax, data, u, cluster_index):
    # Plot points with membership intensities based on the cluster index
    colors = u[cluster_index, :]
    scatter = ax.scatter(data[:, 0], data[:, 1], c=colors, cmap='coolwarm', edgecolor='k', marker='o')
...
def plot_cluster_circles(ax, centers, color='r', linewidth=2):
    for center in centers:
        circle = plt.Circle(center, color=color, fill=False, linewidth=linewidth)
...
axes[0, 0].scatter(data[:, 0], data[:, 1], c=kmeans_labels, cmap='viridis', marker='o', edgecolor='k')
axes[0, 0].set_title('C-Means (K-Means) Clustering')
plot_cluster_circles(axes[0, 0], kmeans.cluster_centers_, color='blue')

# Plot FCM membership for cluster 1
axes[1, 0].set_title('Fuzzy C-Means - Cluster 1 Membership')
plot_fcm_membership(axes[1, 0], data, u, 0)

# Plot FCM membership for cluster 2
axes[1, 1].set_title('Fuzzy C-Means - Cluster 2 Membership')
plot_fcm_membership(axes[1, 1], data, u, 1)

# Plot FCM membership for cluster 3
axes[0, 1].set_title('Fuzzy C-Means - Cluster 3 Membership')
plot_fcm_membership(axes[0, 1], data, u, 2)
